...
// ★ システム暗号化復号用：通常のreadFormatInformation（誤り耐性あり）で型式情報を読み取り、
//   マスク番号だけをビット反転して元のマスクを復元する。
//   生成ソフトは反転マスク+正しいBCHで有効な型式情報を書き込んでいるため、
//   通常のreadFormatInformationで正常に読み取れる。あとはマスク番号を反転するだけ。
...
function decodeMatrix(matrix, options) {
    ...
    try {
        ...
        res.codewords = codewords; // 常にcodewordsを添付する
        if (options && options.extractRawForFailed) {
